...
- Grafana stack
  - grafana
  - prometheus
  - VictoriaMetrics cluster and selectable Operator-managed VMAgent collector
...
## Selectable metrics collectors

Prometheus and VictoriaMetrics are installed independently from the active
scraper:

- `prometheus.enabled` installs kube-prometheus-stack and keeps ownership of
  the Prometheus backend when it is needed.
- `victoria_metrics.enabled` installs the VictoriaMetrics storage/query
  cluster. It does not install cluster-wide controllers by itself.
- `victoria_metrics.operator.enabled` installs the VictoriaMetrics Operator
  chart `0.67.2`, its official CRDs/RBAC, and the dependent release that owns
  VMAgent and native scrape resources. It defaults to `false`.
- `kube_state_metrics.enabled` installs the independent kube-state-metrics
  release and defaults to `true`.
- `node_exporter.enabled` installs the independent node-exporter release and
  defaults to `true`. Neither exporter is owned by kube-prometheus-stack.
- `metrics_collector` selects exactly one active scraper: `prometheus` (the
  default) or `victoria_metrics`.

The selected backend must be enabled, but the other backend is optional:

| `metrics_collector` | Prometheus | VictoriaMetrics | VM Operator | Result |
|---|---:|---:|---:|---|
| `prometheus` | enabled | optional | optional | Prometheus is the only scraper |
| `victoria_metrics` | optional | enabled | enabled | VMAgent is the only scraper |

Both backends can remain installed during migration. The Prometheus converter
is enabled only while Prometheus, VictoriaMetrics, and the VM Operator are all
enabled. With `victoria_metrics.enabled = true` and
`victoria_metrics.operator.enabled = false`, VictoriaMetrics remains available
only as a remote-write/query backend and no VM Operator CRDs or cluster-wide
RBAC are installed. Selecting `metrics_collector = "victoria_metrics"` without
enabling the Operator fails during planning. In a clean VictoriaMetrics-only
installation the converter is disabled, and no module-owned object uses the
`monitoring.coreos.com` API group.

### In-place upgrade warning

> [!WARNING]
> Upgrading an existing kube-prometheus-stack installation changes live Helm
> ownership and Kubernetes object identities. A Terraform plan normally shows
> only the parent `helm_release` updates, not every object Helm will delete or
> recreate. Perform the first migration as one complete root-module plan and
> apply. Targeted (`-target`), split, or otherwise partial applies are not a
> supported migration path.

The migration has three externally visible effects:

- the new `victoria_metrics.operator.enabled` gate defaults to `false`. If an
  earlier configuration already installed the VM Operator, set it explicitly
  to `true` before upgrading to preserve the Operator, its CRDs/RBAC, and the
  dependent custom-resource release. Setting it to `false` removes both Helm
  releases but leaves the VictoriaMetrics cluster and vmstorage PVCs managed by
  `victoria_metrics.enabled`. The module rejects disabling it while
  `metrics_collector = "victoria_metrics"`;
- kube-state-metrics moves from the `prometheus` Helm release to the standalone
  `kube-state-metrics` release. Its default resource fullname stays
  `prometheus-kube-state-metrics`, but its
  `meta.helm.sh/release-name` ownership changes. The root dependency orders the
  Prometheus chart update before the standalone install so the old subchart
  objects are removed first. An interrupted or partial apply can leave a
  same-name object with stale ownership and make Helm report
  `invalid ownership metadata`.
- node-exporter moves from the kube-prometheus-stack resource fullname
  `prometheus-prometheus-node-exporter` to the standalone fullname
  `prometheus-node-exporter`. Expect the old DaemonSet/Service identity to be
  replaced and allow for a short node-metric scrape gap during convergence.
- later changing `metrics_collector` to `victoria_metrics` while keeping
  `prometheus.enabled = true` leaves the kube-prometheus-stack Helm release,
  Operator, and configured Alertmanager installed, but sets the chart's
  Prometheus server to disabled. Helm removes the `Prometheus` custom resource,
  and the Operator removes its generated StatefulSet. With the current storage
  contract, existing Prometheus PVCs are expected to remain, but the Prometheus
  query endpoint and its history are unavailable until the server is enabled
  again. Never infer PVC preservation only from the Terraform Helm diff.

Before the first migration apply, record the current objects and exact PVC
names (replace `monitoring` if a custom namespace is used):
...
After each apply, repeat those commands and confirm that expected Prometheus
PVC names still exist. Do not delete Prometheus or VictoriaMetrics PVCs during
the migration or its rollback.

If the standalone kube-state-metrics install fails with an ownership error,
stop targeted retries and first finish or rerun the complete root-module apply
so the `prometheus` release can remove its bundled subchart. If the full apply
still fails, inspect the exact colliding object's
`meta.helm.sh/release-name` and `meta.helm.sh/release-namespace` annotations.
Only after confirming that the object is stale, still owned by the old release,
and no longer rendered by that release should an operator delete that exact
object and rerun the full apply. Do not bulk-delete kube-state-metrics objects,
PVCs, or CRDs as recovery.

For a standalone VictoriaMetrics stack:
...
This keeps the independent kube-state-metrics and node-exporter releases,
creates native `VMServiceScrape` resources for them, and creates authenticated
`VMNodeScrape` resources for kubelet and cAdvisor. The optional
`/metrics/resource` endpoint is disabled by default. Known node metric patterns
are filtered per endpoint; custom caller patterns remain enabled on every
selected node endpoint, while KSM/scheduler metrics stay on their separate
collection paths. It also replaces the component discovery previously owned
by kube-prometheus-stack. CoreDNS,
kube-proxy, and etcd are enabled safe defaults; controller-manager and
scheduler are supported after their TLS settings are supplied. API-server
scraping is opt-in because this module's Prometheus configuration disables it
by default.
Service-account credentials are sent only to authenticated HTTPS endpoints;
plain HTTP component targets receive no Authorization token.
For self-managed control planes whose serving certificates use a different CA
or SAN, set `controller_manager_tls` / `scheduler_tls` under
`kubernetes_component_scrapes` and mount that CA into VMAgent through
`agent.extra_configs`. `insecure_skip_verify = true` is available only as an
explicit compatibility opt-in. A rendered scrape is not proof that the target
certificate matches these defaults; confirm target health after apply.

To install both backends while Prometheus remains active and VictoriaMetrics
receives a validation copy:
...
In this Prometheus-first state, Prometheus is the one active scraper and
remote-writes a validation copy to VictoriaMetrics. The independent
kube-state-metrics release creates its Prometheus `ServiceMonitor`. The
Operator is installed, but no `VMAgent` object is created yet, so targets are
not scraped twice. The monitor drops kube-state-metrics Go runtime metrics
matching `^go_.*`. Grafana provisions both datasources and defaults to
Prometheus; select VictoriaMetrics to verify the copied data.

After validation, switch the active scraper with:

```hcl
metrics_collector = "victoria_metrics"
```

Terraform disables the Prometheus server if that release remains installed:
the `Prometheus` custom resource and its Operator-generated StatefulSet are
removed while retained PVCs must be verified as described above. Terraform also
creates exactly one Operator-managed `VMAgent` object. Its name
and replicas come from `victoria_metrics.agent.name` and
`victoria_metrics.agent.replica_count`; it writes to the existing vminsert
service. The kube-state-metrics Prometheus `ServiceMonitor` is disabled and
the module creates exactly one native `VMServiceScrape` with port `http`,
`honorLabels: true`, a scoped `max_scrape_size: 32MiB` endpoint limit, and the
equivalent `^go_.*` metric drop. Its name is
`<kube-state-metrics Service fullname>-victoria-metrics`, avoiding a Helm
ownership collision with the same-name object converted from the old
`ServiceMonitor` during handoff.

If `victoria_metrics.agent.extra_scrape_configs` contains a job named exactly
`kube-state-metrics`, that caller job is preserved and suppresses the native
object. The caller then owns its scrape-size limit. Other inline jobs are
preserved unchanged.

When an inline job replaces any module-owned kube-state-metrics,
node-exporter, Tempo, or Loki target, set its matching
`victoria_metrics.agent.managed_service_scrapes` field to `false`. This
suppresses only the generated VictoriaMetrics `VMServiceScrape`; it does not
uninstall the workload, change Prometheus-mode discovery, or remove the caller
job. All four fields default to `true`. The exact `kube-state-metrics` job-name
behavior above remains supported for existing migration configurations.

During dual-backend migration, the application may temporarily retain
`PodMonitor`/`ServiceMonitor` resources and their Secret selectors; the
Operator converter reads them at runtime. Before setting
`prometheus.enabled = false`, every application-owned `PodMonitor`,
`ServiceMonitor`, `PrometheusRule`, authenticated endpoint, and additional
scrape job must have a validated native VictoriaMetrics equivalent. Terraform
does not translate application monitors or copy Secret values. Inline jobs
must not contain tokens, passwords, or other credentials.

The native Kubernetes component pairs are rendered only when VictoriaMetrics
is standalone (`prometheus.enabled = false`). If both backends are installed,
the module renders no native component pair and the VictoriaMetrics Operator
converter consumes kube-prometheus-stack's component `ServiceMonitor`
resources instead. This avoids scraping the same endpoint twice.

On managed EKS, CoreDNS and kube-proxy normally have discoverable targets.
Controller-manager, scheduler, and etcd run in the AWS-managed control plane,
so their generated Services can legitimately have no endpoints. On a
self-managed control plane, matching pods in the configured component
namespace are discovered automatically. The status output reports which
objects Terraform renders; it is not a VMAgent target-health signal.

The Operator chart's pinned `0.67.2` chart-owned ClusterRole grants
cluster-wide wildcard verbs on `secrets` and `secrets/finalizers`, because
runtime conversion needs Secret reads. Restrict and audit the Operator service
account and access to both source and generated configuration Secrets. A
narrower custom RBAC design is a separate hardening change.

Selector-owned Operator/VMAgent fields are applied after caller values. The
module protects the conversion ownership flags, empty `watchNamespaces`,
empty `controller.disableReconcileFor`, VMAgent identity/activation,
`selectAllByDefault`, remote-write destination, inline scrape config, and
monitor selector fields. It filters `WATCH_NAMESPACE` and
`VM_ENABLEDPROMETHEUSCONVERTER*` env entries, forces `envFrom = []`, and does
not accept raw Pod/Service scrape-selector overrides. Partial resources and
`extraArgs` overrides retain unspecified defaults.

The Operator chart uses protected `crds.enabled = true` and
`crds.plain = true`. It installs and upgrades official CRDs with cleanup
disabled. A separate module-local resources release depends on the Operator
release and creates `VMAgent`, `VMServiceScrape`, and `VMNodeScrape` instances;
this ordering avoids clean-cluster REST-mapping failures.

The VictoriaMetrics cluster release, vmstorage PVC identities, retention, and
historical data are left unchanged. Helm releases converge non-atomically, so
a bounded overlap or collection gap is possible; the switch does not backfill
samples.

Use the following three-apply rollout. Each step is a complete, non-targeted
root-module apply:

1. Apply with both backends enabled and `metrics_collector = "prometheus"`.
   This performs the exporter ownership/identity migration. Verify the
   standalone exporters, recorded PVCs, Prometheus remote-write queue health,
   and converted authenticated targets before switching.
2. Set only `metrics_collector = "victoria_metrics"`, apply during an agreed
   handoff window, verify that the Prometheus PVC names remain present, and
   verify the VMAgent target page and application metrics.
3. After every application-owned monitor/rule/authenticated scrape has a
   verified native replacement, set `prometheus.enabled = false` and apply.

The Prometheus gate is:

```promql
max(prometheus_remote_storage_samples_pending) == 0
```

During the observation window, failed and retried remote-write counters must
be non-increasing (for example,
`increase(prometheus_remote_storage_samples_failed_total[5m]) == 0` and
`increase(prometheus_remote_storage_samples_retried_total[5m]) == 0`).
...
The VMAgent remote-write connection-read, connection-write, dial, retry, and
dropped-packet counters must be non-increasing during the observation window:
...
`vmagent_remotewrite_retries_total`, and
`vmagent_remotewrite_packets_dropped_total`. The default VMAgent queue is
ephemeral, so deleting an agent with pending data can lose those queued
samples.

During step 2, while `prometheus.enabled = true`, rollback is a selector-only
change back to `metrics_collector = "prometheus"`. This recreates the
`Prometheus` custom resource and StatefulSet; verify that the recorded PVCs are
reattached before relying on old Prometheus history. After step 3 has removed
the Prometheus Helm release, rollback must set both
`prometheus.enabled = true` and `metrics_collector = "prometheus"` in the same
full apply.

See `tests/metrics-collector-selection/` and the
[VictoriaMetrics migration guide](docs/victoria-metrics-migration.md).

The independent kube-state-metrics and node-exporter releases remain installed
while their respective `enabled` flags are true. Their lifecycles are separate
from both backends. Prometheus mode creates one `ServiceMonitor` per exporter;
VictoriaMetrics mode creates one native `VMServiceScrape` per exporter.

For agent-specific settings, use `victoria_metrics.agent`:
...
victoria_metrics = {
  ...
  agent = {
    ...
    managed_service_scrapes = {
      # This target is collected by the custom inline job below.
      node_exporter = false
    }
    extra_scrape_configs = [
      {
        job_name       = "custom-node-exporter"
        static_configs = [{ targets = ["prometheus-node-exporter.monitoring.svc.cluster.local:9100"] }]
      }
    ]
  }
}
...
`agent.extra_scrape_configs` is for exceptional targets without a monitor CR.
The generated VMAgent uses `selectAllByDefault = true`. In dual mode, source
Prometheus monitors are converted by the Operator; in standalone mode, native
VictoriaMetrics monitor resources are selected directly. Do not put
credentials in inline configuration. If an inline job replaces a module-owned
service target, disable the matching generated scrape under
`agent.managed_service_scrapes` to prevent duplicate collection.

The generated VictoriaMetrics defaults are `500m` CPU/`512Mi` memory requests and `2` CPU/`1Gi` limits for vminsert; `500m` CPU/`1Gi` memory requests and `1` CPU/`2Gi` limits for vmstorage; plus `1` CPU/`512Mi` requests, `2` CPU/`1Gi` limits, and 16 remote-write queues for VMAgent. `victoria_metrics.extra_configs` can override non-endpoint cluster values. A final protected layer keeps the derived vminsert/vmselect service names, enablement, ports, target ports, and HTTP listen addresses consistent with the generated VMAgent and Grafana URLs. Partial `victoria_metrics.agent.extra_configs.resources` and `.extraArgs` maps can override VMAgent defaults. The module does not enable VMAgent persistent storage; if `/tmpData` is an `emptyDir`, applying a rollout discards any pending remote-write backlog.
...
- <1.27.0 to >=1.27.0
  ...
  - BREAKING: changes in dashboard widget/blocks(make sure to check if dashboard created via code are working ok):
    ...
    - dashboard decimals now shown in charts by default, so instead for example rounded 99% availability we can see 99.9% in SLA
    - better way to calculate dashboard widget placement coordinates, to not have broken dashboards
    - units have been enabled on several widgets like cpu/memory/network
    - issue of correctly handling in UI the passed variables to dashboards, like usual `namespaces` list one got fixed
...
| <a name="module_node_exporter"></a> [node\_exporter](#module\_node\_exporter) | ./modules/node-exporter | n/a |
...
| <a name="input_kube_state_metrics"></a> [kube\_state\_metrics](#input\_kube\_state\_metrics) | Independent kube-state-metrics release shared by the selected metrics collector. | <pre>object({<br/>    enabled           = optional(bool, true)<br/>    namespace         = optional(string, null)<br/>    create_namespace  = optional(bool, true)<br/>    chart_version     = optional(string, "7.8.1")<br/>    release_name      = optional(string, "kube-state-metrics")<br/>    fullname_override = optional(string, null)<br/>    extra_configs     = optional(any, {})<br/>  })</pre> | `{}` | no |
...
| <a name="input_tempo"></a> [tempo](#input\_tempo) | Configs for tempo deployment | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, null) # the namespace fallbacks to var.namespace if not specified<br/>    create_namespace = optional(bool, true)   # whether create namespace if not exist<br/>    chart_version    = optional(string, "1.23.3")<br/>    release_name     = optional(string, "tempo")<br/>    service_account = optional(object({<br/>      name        = optional(string, "tempo")<br/>      annotations = optional(map(string), {})<br/>    }), {})<br/>    storage = optional(object({<br/>      backend = optional(string, "local")<br/>      backend_configuration = optional(map(any), {<br/>        local = { path = "/var/tempo/traces" },<br/>        wal   = { path = "/var/tempo/wal" }<br/>      })<br/>    }), {})<br/>    enable_service_monitor = optional(bool, true)<br/>    oidc_provider_arn      = optional(string, "")<br/><br/>    metrics_generator = optional(object({<br/>      enabled    = optional(bool, true)<br/>      remote_url = optional(string, null)<br/>    }), {})<br/><br/>    persistence = optional(object({<br/>      enabled       = optional(bool, true)<br/>      size          = optional(string, "20Gi")<br/>      storage_class = optional(string, "")<br/>    }), {})<br/>    extra_configs = optional(any, {}) # allows to pass extra/custom configs to tempo helm chart, this configs will deep-merged with all generated internal configs and can override the default set ones. All available options can be found in for the specified chart version here: https://artifacthub.io/packages/helm/grafana/tempo?modal=values<br/>  })</pre> | `{}` | no |
| <a name="input_victoria_metrics"></a> [victoria\_metrics](#input\_victoria\_metrics) | Values to deploy the VictoriaMetrics cluster and Operator-managed VMAgent, and wire the selected metrics collector | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, null) # the namespace fallbacks to var.namespace if not specified<br/>    create_namespace = optional(bool, true)   # whether create namespace if not exist<br/>    chart_version    = optional(string, "0.31.0")<br/>    release_name     = optional(string, "victoria-metrics")<br/>    retention_period = optional(string, "30d")<br/>    vmstorage = optional(object({<br/>      replica_count = optional(number, 3)<br/>      storage_class = optional(string, "")<br/>      storage_size  = optional(string, "100Gi")<br/>      access_modes  = optional(list(string), ["ReadWriteOnce"])<br/>    }), {})<br/>    vminsert = optional(object({<br/>      replica_count = optional(number, 2)<br/>    }), {})<br/>    vmselect = optional(object({<br/>      replica_count = optional(number, 2)<br/>    }), {})<br/>    operator = optional(object({<br/>      enabled       = optional(bool, false)<br/>      chart_version = optional(string, "0.67.2")<br/>      release_name  = optional(string, "victoria-metrics-operator")<br/>      extra_configs = optional(any, {})<br/>    }), {})<br/>    agent = optional(object({<br/>      name                    = optional(string, "victoria-metrics-agent")<br/>      replica_count           = optional(number, 1)<br/>      kubelet_scrape_enabled  = optional(bool, true)<br/>      cadvisor_scrape_enabled = optional(bool, true)<br/>      resource_scrape_enabled = optional(bool, false)<br/>      kubernetes_component_scrapes = optional(object({<br/>        namespace            = optional(string, "kube-system")<br/>        api_server_namespace = optional(string, "default")<br/>        api_server           = optional(bool, false)<br/>        core_dns             = optional(bool, true)<br/>        kube_proxy           = optional(bool, true)<br/>        controller_manager   = optional(bool, false)<br/>        scheduler            = optional(bool, false)<br/>        etcd                 = optional(bool, true)<br/>        controller_manager_tls = optional(object({<br/>          ca_file              = optional(string, null)<br/>          server_name          = optional(string, null)<br/>          insecure_skip_verify = optional(bool, false)<br/>        }), null)<br/>        scheduler_tls = optional(object({<br/>          ca_file              = optional(string, null)<br/>          server_name          = optional(string, null)<br/>          insecure_skip_verify = optional(bool, false)<br/>        }), null)<br/>      }), {})<br/>      managed_service_scrapes = optional(object({<br/>        kube_state_metrics = optional(bool, true)<br/>        node_exporter      = optional(bool, true)<br/>        tempo              = optional(bool, true)<br/>        loki               = optional(bool, true)<br/>      }), {})<br/>      kubelet_metrics = optional(list(string), [<br/>        "container_cpu_.*",<br/>        "container_memory_.*",<br/>        "container_network_.*",<br/>        "pod_cpu_usage_seconds_total",<br/>        "pod_memory_working_set_bytes",<br/>        "kubelet_volume_stats.*",<br/>        "volume_operation_total_seconds.*",<br/>        "container_fs_.*",<br/>      ])<br/>      extra_scrape_configs = optional(any, [])<br/>      extra_configs        = optional(any, {})<br/>    }), {})<br/>    extra_configs = optional(any, {})<br/>  })</pre> | `{}` | no |
...
| <a name="output_kube_state_metrics"></a> [kube\_state\_metrics](#output\_kube\_state\_metrics) | kube-state-metrics sub-module outputs |
| <a name="output_loki"></a> [loki](#output\_loki) | loki sub-module outputs |
| <a name="output_metrics_collector"></a> [metrics\_collector](#output\_metrics\_collector) | The active metrics collector selected by metrics\_collector. |
| <a name="output_metrics_collector_status"></a> [metrics\_collector\_status](#output\_metrics\_collector\_status) | Resolved metrics collector installation and rendered scrape configuration status; scrape booleans do not report runtime target health. |
| <a name="output_node_exporter"></a> [node\_exporter](#output\_node\_exporter) | node-exporter sub-module outputs |
...
| <a name="output_victoria_metrics"></a> [victoria\_metrics](#output\_victoria\_metrics) | VictoriaMetrics child-module outputs. |
...
| <a name="input_skip_folder_creation"></a> [skip\_folder\_creation](#input\_skip\_folder\_creation) | If true, folders are created in submodules. If false, folders are created centrally. | `bool` | `false` | no |
| <a name="input_tempo"></a> [tempo](#input\_tempo) | Configs for tempo deployment | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, null) # the namespace fallbacks to var.namespace if not specified<br/>    create_namespace = optional(bool, true)   # whether create namespace if not exist<br/>    chart_version    = optional(string, "1.23.3")<br/>    release_name     = optional(string, "tempo")<br/>    service_account = optional(object({<br/>      name        = optional(string, "tempo")<br/>      annotations = optional(map(string), {})<br/>    }), {})<br/>    storage = optional(object({<br/>      backend = optional(string, "local")<br/>      backend_configuration = optional(map(any), {<br/>        local = { path = "/var/tempo/traces" },<br/>        wal   = { path = "/var/tempo/wal" }<br/>      })<br/>    }), {})<br/>    enable_service_monitor = optional(bool, true)<br/>    oidc_provider_arn      = optional(string, "")<br/><br/>    metrics_generator = optional(object({<br/>      enabled    = optional(bool, true)<br/>      remote_url = optional(string, null)<br/>    }), {})<br/><br/>    persistence = optional(object({<br/>      enabled       = optional(bool, true)<br/>      size          = optional(string, "20Gi")<br/>      storage_class = optional(string, "")<br/>    }), {})<br/>    extra_configs = optional(any, {}) # allows to pass extra/custom configs to tempo helm chart, this configs will deep-merged with all generated internal configs and can override the default set ones. All available options can be found in for the specified chart version here: https://artifacthub.io/packages/helm/grafana/tempo?modal=values<br/>  })</pre> | `{}` | no |
| <a name="input_victoria_metrics"></a> [victoria\_metrics](#input\_victoria\_metrics) | Values to deploy the VictoriaMetrics cluster and Operator-managed VMAgent, and wire the selected metrics collector | <pre>object({<br/>    enabled          = optional(bool, false)<br/>    namespace        = optional(string, null) # the namespace fallbacks to var.namespace if not specified<br/>    create_namespace = optional(bool, true)   # whether create namespace if not exist<br/>    chart_version    = optional(string, "0.31.0")<br/>    release_name     = optional(string, "victoria-metrics")<br/>    retention_period = optional(string, "30d")<br/>    vmstorage = optional(object({<br/>      replica_count = optional(number, 3)<br/>      storage_class = optional(string, "")<br/>      storage_size  = optional(string, "100Gi")<br/>      access_modes  = optional(list(string), ["ReadWriteOnce"])<br/>    }), {})<br/>    vminsert = optional(object({<br/>      replica_count = optional(number, 2)<br/>    }), {})<br/>    vmselect = optional(object({<br/>      replica_count = optional(number, 2)<br/>    }), {})<br/>    operator = optional(object({<br/>      enabled       = optional(bool, false)<br/>      chart_version = optional(string, "0.67.2")<br/>      release_name  = optional(string, "victoria-metrics-operator")<br/>      extra_configs = optional(any, {})<br/>    }), {})<br/>    agent = optional(object({<br/>      name                    = optional(string, "victoria-metrics-agent")<br/>      replica_count           = optional(number, 1)<br/>      kubelet_scrape_enabled  = optional(bool, true)<br/>      cadvisor_scrape_enabled = optional(bool, true)<br/>      resource_scrape_enabled = optional(bool, false)<br/>      kubernetes_component_scrapes = optional(object({<br/>        namespace            = optional(string, "kube-system")<br/>        api_server_namespace = optional(string, "default")<br/>        api_server           = optional(bool, false)<br/>        core_dns             = optional(bool, true)<br/>        kube_proxy           = optional(bool, true)<br/>        controller_manager   = optional(bool, false)<br/>        scheduler            = optional(bool, false)<br/>        etcd                 = optional(bool, true)<br/>        controller_manager_tls = optional(object({<br/>          ca_file              = optional(string, null)<br/>          server_name          = optional(string, null)<br/>          insecure_skip_verify = optional(bool, false)<br/>        }), null)<br/>        scheduler_tls = optional(object({<br/>          ca_file              = optional(string, null)<br/>          server_name          = optional(string, null)<br/>          insecure_skip_verify = optional(bool, false)<br/>        }), null)<br/>      }), {})<br/>      kubelet_metrics = optional(list(string), [<br/>        "container_cpu_.*",<br/>        "container_memory_.*",<br/>        "container_network_.*",<br/>        "pod_cpu_usage_seconds_total",<br/>        "pod_memory_working_set_bytes",<br/>        "kubelet_volume_stats.*",<br/>        "volume_operation_total_seconds.*",<br/>        "container_fs_.*",<br/>      ])<br/>      extra_scrape_configs = optional(any, [])<br/>      extra_configs        = optional(any, {})<br/>    }), {})<br/>    extra_configs = optional(any, {})<br/>  })</pre> | `{}` | no |
