...
**Created**: 2026-07-23
**Status**: Draft
**Input**: User description: "Refine disk alerting globally, not for a specific client. Determine whether disk alerts are needed and, if yes, implement them for everyone. Use VictoriaMetrics for all module-generated alerts and dashboards when available."
...
4. **Given** VictoriaMetrics is enabled, **When** application dashboard widgets and dashboard block alerts are generated, **Then** their Prometheus-compatible metric datasource defaults use the VictoriaMetrics datasource UID.
...
- VictoriaMetrics may be disabled in some module deployments; generated metric alerts and dashboards should fall back to the Prometheus datasource rather than referencing a missing UID.
...
- Caller-provided custom alert rules and explicit dashboard datasource overrides must continue to be preserved alongside module-provided defaults.
...
- **FR-004**: Module-generated Prometheus-compatible alert defaults MUST use VictoriaMetrics datasource UID `victoriametrics` when VictoriaMetrics is enabled.
- **FR-005**: Module-generated Prometheus-compatible dashboard defaults MUST use VictoriaMetrics datasource UID `victoriametrics` when VictoriaMetrics is enabled.
...
- **FR-010**: The bundled stack dashboard datasource variable MUST default to `victoriametrics` while preserving existing panel query definitions.
- **FR-011**: Generated metric alert and dashboard defaults MUST fall back to Prometheus datasource UID `prometheus` when VictoriaMetrics is not enabled.
- **FR-012**: New Terraform examples and documentation MUST avoid client-specific names.
...
- **Datasource Selection**: Shared metric datasource default that chooses `victoriametrics` when enabled, falls back to `prometheus`, and can be overridden where the module exposes datasource controls.
...
- **SC-003**: The generated disk alert, dashboard block alerts, and application dashboard metric widgets use `victoriametrics` when VictoriaMetrics is enabled and `prometheus` when it is disabled.
...
- Changing dashboard query expressions.
- Forcing caller-provided custom alert rules or explicit per-widget datasource overrides away from their configured datasource.
