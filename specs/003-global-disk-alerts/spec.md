...
**Created**: 2026-07-23
**Status**: Draft
**Input**: User description: "Refine disk alerting globally, not for a specific client. Determine whether disk alerts are needed and, if yes, implement them for everyone. Use VictoriaMetrics as the alert datasource when available."
...
3. **Given** VictoriaMetrics is enabled, **When** the disk alert rule is generated, **Then** the rule uses the VictoriaMetrics datasource UID.
...
- **Datasource Selection**: Module behavior that chooses `victoriametrics` when enabled and `prometheus` otherwise.
