module "base" {
  ...
  metrics = [
    ...
    { label = "Avg({{pod}})", color = "FFC300", expression = "avg(container_memory_usage_bytes{container=\"${var.container}\", namespace=\"${var.namespace}\"}) by (pod)" },
    { label = "Max({{pod}})", color = "FF774D", expression = "max(container_memory_usage_bytes{container=\"${var.container}\", namespace=\"${var.namespace}\"}) by (pod)" },
  ]
}
