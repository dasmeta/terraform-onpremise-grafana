output "result" {
  ...
  value = [
    ...
    [
      ...
      { type : "msk/disk", cluster_names = var.cluster_names, broker_ids = var.broker_ids, width = 8, region = var.region, datasource_uid = var.datasource_uid, period = var.period },
    ],
    [
    ...
    [
      { type : "msk/offline_partitions", cluster_names = var.cluster_names, width = 12, region = var.region, datasource_uid = var.datasource_uid, period = var.period },
      { type : "msk/under_replicated", cluster_names = var.cluster_names, broker_ids = var.broker_ids, width = 12, region = var.region, datasource_uid = var.datasource_uid, period = var.period },
    ],
    [
      { type : "msk/consumer_lag", cluster_names = var.cluster_names, consumer_groups = var.consumer_groups, topics = var.topics, width = 24, region = var.region, datasource_uid = var.datasource_uid, period = var.period },
    ],
  ]
}
