output "result" {
  ...
  value = [
    ...
    var.show_err_logs ?
    [
      { type : "deployment/errors", deployment : var.name, namespace = var.namespace, datasource_uid = var.loki_datasource_uid, width : 24, expr = var.expr },
    ] :
    [],
    concat(
      [
        { type : "container/network", host : var.host, container : var.name, namespace = var.namespace, width : var.host != null ? 6 : 24, datasource_uid = var.prometheus_datasource_uid },
        { type : "container/network-error", host : var.host, pod : var.name, namespace = var.namespace, width : var.host != null ? 6 : 24, datasource_uid = var.prometheus_datasource_uid },
      ],
      var.host != null ? [
