...
locals {
  widget_alert_rules = concat(flatten(values(module.block_sla_nginx_alerts).*.alert_rules), flatten(values(module.block_ingress_nginx_alerts).*.alert_rules), flatten(values(module.block_service_alerts).*.alert_rules)) #, flatten(values(module.block_ingress_nginx_alerts).*.alert_rules), flatten(values(module.block_service_alerts).*.alert_rules)) #concat(concat([], values(module.block_sla_nginx_alerts).*.alert_rules...), concat([], values(module.block_ingress_nginx_alerts).*.alert_rules...), concat([], values(module.block_service_alerts).*.alert_rules...))
