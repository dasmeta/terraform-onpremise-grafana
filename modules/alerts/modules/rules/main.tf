locals {
  ...
  alerts = { for member in local.groups : member => [for rule in var.alert_rules : merge(rule, {
    expr : coalesce(rule.expr, "${rule.metric_function}(${rule.metric_name}${rule.filters != null ? replace(jsonencode(rule.filters), "\":\"", "\"=\"" /*replace(join(", ", [for k, v in rule.filters : "${k}=\"${v}\""]), "\"", "\\\"")*/) : ""}${rule.metric_interval})")
